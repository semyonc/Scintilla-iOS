...
sptr_t ScintillaBase::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_AUTOCSHOW:
		listType = 0;
		AutoCompleteStart((int)wParam, reinterpret_cast<const char *>(lParam));
		break;
	...
	case SCI_AUTOCSETCASEINSENSITIVEBEHAVIOUR:
		ac.ignoreCaseBehaviour = (int)wParam;
		break;
	...
	case SCI_AUTOCSETMAXHEIGHT:
		ac.lb->SetVisibleRows((int)wParam);
		break;
	...
	case SCI_REGISTERIMAGE:
		ac.lb->RegisterImage((int)wParam, reinterpret_cast<const char *>(lParam));
		break;

	case SCI_REGISTERRGBAIMAGE:
		ac.lb->RegisterRGBAImage((int)wParam, sizeRGBAImage.x, sizeRGBAImage.y, reinterpret_cast<unsigned char *>(lParam));
		break;
	...
	case SCI_CALLTIPSHOW:
		CallTipShow(LocationFromPosition((int)wParam),
	...
	case SCI_CALLTIPSETHLT:
		ct.SetHighlight((int)wParam, (int)lParam);
		break;
	...
	case SCI_COLOURISE:
		if (DocumentLexState()->lexLanguage == SCLEX_CONTAINER) {
			pdoc->ModifiedAt((int)wParam);
			NotifyStyleToNeeded((lParam == -1) ? pdoc->Length() : (int)lParam);
		} else {
			DocumentLexState()->Colourise((int)wParam, (int)lParam);
		}
		Redraw();
		break;
	...
	case SCI_GETPROPERTYINT:
		return DocumentLexState()->PropGetInt(reinterpret_cast<const char *>(wParam), (int)lParam);
	...
	case SCI_PRIVATELEXERCALL:
		return reinterpret_cast<sptr_t>(
			DocumentLexState()->PrivateCall((int)wParam, reinterpret_cast<void *>(lParam)));
