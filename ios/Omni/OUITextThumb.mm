...
- (id)initWithFrame:(CGRect)frame;
{
    ...
    ascent = -1;
    width = -1;
    shouldRecognizeSimultaneous = YES;
    ...
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(_inspect:)];
...
{
    return shouldRecognizeSimultaneous;
}

@synthesize shouldRecognizeSimultaneous;
@synthesize isEndThumb;
@synthesize delegate = nonretained_editor;
...
- (void)_dragged:(UIPanGestureRecognizer *)gestureRecognizer;
{
    ...
    CGPoint delta = [gestureRecognizer translationInView:nonretained_editor];
    ...
    if (st == UIGestureRecognizerStateBegan) {
        ...
        touchdownPoint = [self convertPoint:(CGPoint){0, - ascent/2} toView:nonretained_editor];
        [nonretained_editor thumbBegan:self];
    }
    ...
    if ((isfinite(delta.x) && isfinite(delta.y)) &&
        ...
        [nonretained_editor thumbMoved:self targetPosition:(CGPoint){ touchdownPoint.x + delta.x, touchdownPoint.y + delta.y }];
    }
    
    if (st == UIGestureRecognizerStateEnded || st == UIGestureRecognizerStateCancelled) {
        [nonretained_editor thumbEnded:self normally:(st == UIGestureRecognizerStateEnded? YES:NO)];
        touchdownPoint = (CGPoint){ NAN, NAN };
    }
}

- (void)_inspect:(UIPanGestureRecognizer *)gestureRecognizer
{
    UIGestureRecognizerState st = gestureRecognizer.state;

    if (st == UIGestureRecognizerStateBegan) {
        [nonretained_editor tumbInspect:self];
    } else if (st == UIGestureRecognizerStateEnded) {
        [nonretained_editor thumbEnded:self normally:YES];
    }
}
