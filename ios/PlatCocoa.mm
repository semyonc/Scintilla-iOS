 ...
 */
CGRect PRectangleToCGRect(PRectangle& rc)
{
 ...
 */
PRectangle CGRectToPRectangle(CGRect& rc)
{
                                      ...
                                      ColourDesired fore)
{
    ...
    CGFloat colorRef[4];
    colorRef[0] = colour.GetRed()/255.0;
    colorRef[1] = colour.GetGreen()/255.0;
    colorRef[2] = colour.GetBlue()/255.0;
    colorRef[3] = 1.0;
...
void Window::Destroy()
{
  if (wid)
  {
    ...
    if ([idWin isKindOfClass: [UIWindow class]])
    {
      UIWindow* win = reinterpret_cast<UIWindow*>(idWin);
      [win release];
    }
  }
  wid = 0;
...
{
  UIView* container = reinterpret_cast<UIView*>(wid);
  return [container isFirstResponder];
}

//--------------------------------------------------------------------------------------------------

static int ScreenMax(UIWindow* win)
{
...
PRectangle Window::GetPosition()
{
  if (wid)
  {
    ...
    {
        // NSWindow
        win = reinterpret_cast<UIWindow*>(idWin);
        rect = [win frame];
    }
    else
    {
      // NSView
      UIView* view = reinterpret_cast<UIView*>(idWin);
      win = [view window];
      rect = [view convertRect: [view bounds] toView: nil];
      rect.origin = [win convertPoint:rect.origin toWindow:nil];
    }

    int screenHeight = ScreenMax(win);
    // Invert screen positions to match Scintilla
    return PRectangle(
        CGRectGetMinX(rect), screenHeight - CGRectGetMaxY(rect),
        CGRectGetMaxX(rect), screenHeight - CGRectGetMinY(rect));
  }
  else
  {
...
void Window::SetPosition(PRectangle rc)
{
  if (wid)
  {
    ...
    if ([idWin isKindOfClass: [UIWindow class]])
    {
        ...
        UIWindow* win = reinterpret_cast<UIWindow*>(idWin);
        int screenHeight = ScreenMax(win);
        CGRect nsrc = CGRectMake(rc.left, screenHeight - rc.bottom,
                                 rc.Width(), rc.Height());
        [win setFrame: nsrc];
    }
    else
    {
      ...
      CGRect nsrc = CGRectMake(rc.left, rc.bottom, rc.Width(), rc.Height());
      UIView* view = reinterpret_cast<UIView*>(idWin);
      nsrc.origin = [[view window] convertPoint:nsrc.origin toWindow:nil];
      [view setFrame: nsrc];
    }
  }
}
...
void Window::InvalidateAll()
{
  ...
  {
    id idWin = reinterpret_cast<id>(wid);
    UIView* container = reinterpret_cast<UIView*>(idWin);
...
void Window::InvalidateRectangle(PRectangle rc)
{
  if (wid)
  {
    id idWin = reinterpret_cast<id>(wid);
    UIView* container = reinterpret_cast<UIView*>(idWin);
      [container setNeedsDisplayInRect: PRectangleToCGRect(rc)];
  }
}
...
static UIImage* ImageFromXPM(XPM* pxpm)
{
  UIImage* img = nil;
  if (pxpm)
  {
    ...
    if (surfaceXPM)
    {
      ...
      img = [[[UIImage alloc] initWithCGImage:imageRef] autorelease];
...
public:
  virtual int Rows() = 0;
  virtual UIImage* ImageForRow(NSInteger row) = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (!box)
		return nil;
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"autoCell"];
    if (cell == nil) {
        cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"autoCell"] autorelease];
    }
    if (font != nil)
        cell.textLabel.font = font;
    cell.textLabel.text = box->TextForRow(indexPath.row);
    cell.imageView.image = box->ImageForRow(indexPath.row);
    return cell;
}
...
typedef std::map<NSInteger, UIImage*> ImageMap;
...
//  CGRect lbRect = CGRectMake(pt.x,pt.y, 120, lineHeight * desiredVisibleRows);
//  CGRect scRect = NSMakeRect(0, 0, lbRect.size.width, lbRect.size.height);
...
void ListBoxImpl::SetFont(Font& font_)
{
  ...
  UIFont *pfont = (UIFont *)style->getFontRef();
  ds.font = pfont;
  CGFloat itemHeight = pfont.lineHeight;
...
PRectangle ListBoxImpl::GetDesiredRect()
{
  ...
  int itemHeight = [table estimatedRowHeight];
...
{
  [table selectRowAtIndexPath: [NSIndexPath indexPathWithIndex:n] animated:NO scrollPosition:UITableViewScrollPositionNone];
}

int ListBoxImpl::GetSelection()
{
   return (int)[table indexPathForSelectedRow].row;
}
...
void ListBoxImpl::RegisterRGBAImage(int type, int width, int height, const unsigned char *pixelsImage) {
	...
	UIImage *img = [[[UIImage alloc] initWithCGImage:imageRef] autorelease];
	CGImageRelease(imageRef);
	[img retain];
	ImageMap::iterator it=images.find(type);
	if (it == images.end())
...
UIImage* ListBoxImpl::ImageForRow(NSInteger row)
{
...
{
  UIFont* system = [UIFont systemFontOfSize:[UIFont systemFontSize]];
  NSString* name = system.fontName;
...
{
  return static_cast<int>([UIFont systemFontSize]);
}
...
{
  return 0; // iOS have no double click
}
